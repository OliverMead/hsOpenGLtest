...
bufferSizeCallback theWindow containerWidth containerHeight = do
 ...
 where
  ...
  newDisplayWidth | containerTooTall = round $ scaledWidth / displayScalex
                  | otherwise        = round scaledHeight
  newDisplayHeight :: GL.GLsizei
  newDisplayHeight | containerTooWide = round $ scaledHeight / displayScaley
                   | otherwise        = round scaledWidth
  xoffset :: GL.GLsizei
  xoffset
    | containerTooWide
    = (round . fromIntegral $ containerWidth `div` 2) - newDisplayWidth `div` 2
    | otherwise
    = 0
  yoffset :: GL.GLsizei
  yoffset
